...
import {
  loginWithPhone, onAuthChange, getUserProfile, updateUserProfile,
  searchUserByPhone, setOnline, setOffline, listenPresence,
...
export default function App() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const user = await loginWithPhone(phoneNumber);
      const profile = await getUserProfile(user.uid);
      setUserId(user.uid);
      setUserProfile(profile);
      setOnline(user.uid);
    ...
    return (
      <div className="min-h-screen bg-[#f0f2f5] flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-gray-500 mb-8">Enter your phone number to continue</p>
          <form onSubmit={handleLogin} className="space-y-4">
            <input
              type="tel"
              placeholder="+234 801 234 5678"
              className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#008751] transition-all text-lg"
              value={phoneNumber}
            ...
            <button
              ...
              className="w-full bg-[#008751] text-white font-bold py-3 rounded-xl hover:bg-[#006b40] transition-colors shadow-lg active:scale-[0.98] disabled:opacity-60"
            >
              {loginLoading ? 'Connecting...' : 'Start Messaging'}
            </button>
          </form>
